...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthValue>(() => ({ user, loading, mustChangePassword: Boolean(user?.must_change_password), signIn: async (email, password) => { const token = await apiLogin(email.trim(), password); const current = await api.me(); setUser({ ...current, must_change_password: token.must_change_password }); return token.must_change_password; }, signOut: async () => { await apiLogout(); qc.clear(); setUser(null); }, refreshUser }), [user, loading, qc]);
